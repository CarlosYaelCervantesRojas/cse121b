...
function renderTasks(tasks) {
  ...
  tasksHTLM = tasks.map(
    task => `<li ${task.completed ? 'class="strike"' : ""}>
    <p>${task.detail}</p>
    <div>
  ...
  let tasksString = tasksHTLM.reduce((text, task) => text += task);
  todoList.innerHTML = tasksString;
}
...
function newTask() {
  ...
  tasks.push({detail: valueTodo, completed: false});
  // render out the list.
  renderTasks(tasks);
  todo.value = "";
}
...
function manageTasks(event) {
  ...
  let property = button.dataset.function;
